...
fn build_ui(app: &Application, state: Rc<RefCell<AppState>>) {
    ...
    window.connect_enter_notify_event(move |_, event| {
        ...
        // Ignore spurious crossing events generated by compositor during strip reposition
        if s.last_toggle.elapsed() < COOLDOWN {
            println!("[DEBUG] Ignoring enter event during cooldown");
        ...
        if !s.bar_visible {
            ...
            if s.hide_timer_id.is_none() {
                ...
                let id = glib::timeout_add_local(Duration::from_millis(delay as u64), move || {
                    let mut st = state_clone_timer.borrow_mut();
                    println!("[DEBUG] Hide timer fired.");
                    if st.bar_visible {
                        toggle_waybar();
                        st.bar_visible = false;
                        st.last_toggle = Instant::now();
